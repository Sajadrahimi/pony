...
from pony.orm.ormtypes import (
	LongStr, LongUnicode, raw_sql, RawSQL, normalize, Json, TrackedValue, QueryType,
	...
	def __init__(db_session, retry=0, immediate=False, ddl=False, serializable=False, strict=False, optimistic=True,
		...
		if retry != 0:
			if type(retry) is not int: throw(TypeError,
											 "'retry' parameter of db_session must be of integer type. Got: %s" % type(
												 retry))
...
class Database(object):
	...
	def __init__(self, *args, **kwargs):
		...
		self.AutodiscoverEntity = type.__new__(AutodiscoverEntityMeta, 'AutodiscoverEntity', (AutodiscoverEntity,), {})
		self.AutodiscoverEntity._database_ = self
			...
			for name, val in diff.items():
				if name not in ('class', '_pk_', '_id_', '_status_'):
					...
					if not attr.is_collection:
						if type(val) is dict:
							if 'old' in val: oldvals[attr.name] = oldadict[attr] = attr.validate(
								id2obj(attr, val['old']))
							if 'new' in val: newvals[attr.name] = newadict[attr] = attr.validate(
								id2obj(attr, val['new']))
						else:
							newvals[attr.name] = newadict[attr] = attr.validate(id2obj(attr, val))
			oid = diff['_id_']
...
class Discriminator(Required):
	...
	def process_entity_inheritance(attr, entity):
		...
		if discr_value is not None:
			try:
				entity._discriminator_ = discr_value = attr.validate(discr_value, None, entity)
			except ValueError:
				throw(TypeError,
	...
	def validate(attr, val, obj=None, entity=None, from_db=False):
		...
		if val != entity._discriminator_:
			for cls in entity._subclasses_:
				...
				throw(TypeError, 'Invalid discriminator attribute value for %s. Expected: %r, got: %r'
	...
	def _init_(index, entity):
		...
		for i, attr in enumerate(attrs):
			...
			if not attr.is_required:
				if attr.nullable is False:
					throw(TypeError,
						  'Optional attribute %s must be nullable, because it is part of composite key' % attr)
				attr.nullable = True
		...
		if attr.reverse_column is not None:
			...
			for reverse_column in attr.reverse_columns:
				if not isinstance(reverse_column, basestring):
					throw(TypeError,
						  "Parameter 'reverse_columns' must be a list of strings. Got: %r" % attr.reverse_columns)
...
class EntityMeta(type):
	...
	def __init__(entity, name, bases, cls_dict):
		...
		for name, attr in items_list(entity.__dict__):
			if name in base_attrs_dict: throw(ERDiagramError,
		...
		if len(primary_keys) > 1:
			throw(ERDiagramError, 'Only one primary key can be defined in each entity class')
	...
	def _link_reverse_attrs_(entity):
		...
		for attr in entity._new_attrs_:
			...
			if isinstance(reverse, basestring):
				...
				if attr2 is None: throw(ERDiagramError,
										'Reverse attribute %s.%s not found' % (entity2.__name__, reverse))
			elif isinstance(reverse, Attribute):
				attr2 = reverse
				if attr2.entity is not entity2: throw(ERDiagramError,
	...
	def get_for_update(entity, *args, **kwargs):
		...
		try:
			return entity._find_one_(kwargs, True, nowait, skip_locked)  # can throw MultipleObjectsFoundError
		except ObjectNotFound:
			return None

	@cut_traceback
	def get_by_sql(entity, sql, globals=None, locals=None):
		objects = entity._find_by_sql_(1, sql, globals, locals,
									   frame_depth=cut_traceback_depth + 1)  # can throw MultipleObjectsFoundError
	...
	def select(entity, *args, **kwargs):
		if args:
			query = entity._query_from_args_(args, kwargs, frame_depth=cut_traceback_depth + 1)
		else:
		...
		for attr, attr_is_none in sorted_query_attrs:
			if not attr.reverse:
				if attr_is_none:
					...
					converter = attr.converters[0]
					where_list.append(
						[converter.EQ, ['COLUMN', None, attr.column], ['PARAM', (attr, None, None), converter]])
			elif not attr.columns:
				throw(NotImplementedError)
			else:
				...
				if attr_is_none:
					...
					for j, (column, converter) in enumerate(izip(attr.columns, attr_entity._pk_converters_)):
						where_list.append(
							[converter.EQ, ['COLUMN', None, column], ['PARAM', (attr, None, j), converter]])

		if not for_update:
			sql_ast = ['SELECT', select_list, from_list, where_list]
		else:
			sql_ast = ['SELECT_FOR_UPDATE', nowait, skip_locked, select_list, from_list, where_list]
		...
		for attr in entity._pk_attrs_:
			if attr.column is not None:
				...
				if not attr.reverse:
					...
					val = attr.py_type._get_by_raw_pkval_((val,), from_db=from_db, seed=seed)
			else:
			...
			if exclude:
				...
				for attrname in exclude:
					if attrname not in entity._adict_: throw(AttributeError,
															 'Entity %s does not have attriute %s' % (
...
class AutodiscoverEntityMeta(EntityMeta):
	def __init__(entity, name, bases, cls_dict):
		e = super().__init__(name, bases, cls_dict)
			...
			for arg in args:
				if isinstance(arg, basestring):
					...
					if not isinstance(obj, attr.entity): throw(AttributeError,
															   'Attribute %s does not belong to object %s' % (
															   attr, obj))
				else:
...
class AutodiscoverEntity(AutodiscoverEntityMeta):
	pass
	# def __init__(self, *args, **kwargs):
...
class Query(object):
	def __init__(query, code_key, tree, globals, locals, cells=None, left_join=False):
		...
		if translator is None:
			...
			if name_path:
				tree_copy = unpickle_ast(pickled_tree)  # tree = deepcopy(tree)
				try:
					translator = translator_cls(tree_copy, None, code_key, filter_num, extractors, vars,
												vartypes.copy(),
			...
			if translator.func_extractors_map:
				for func, func_extractors in iteritems(translator.func_extractors_map):
					...
					func_vars, func_vartypes = extract_vars(
						func_id, func_filter_num, func_extractors, func.__globals__, {},
						func.__closure__)  # todo closures
	...
	def _construct_sql_and_arguments(query, limit=None, offset=None, range=None, aggr_func_name=None,
									 aggr_func_distinct=None, sep=None):
	...
	def _order_by(query, method_name, *args):
		...
		if args[0] is None:
			if len(args) > 1: throw(TypeError,
									'When first argument of %s() method is None, it must be the only argument' % method_name)
			tup = (('without_order',),)
		...
		for arg in args:
			if isinstance(arg, int_types):
				...
				throw(TypeError, "order_by() method receive an argument of invalid type: %r" % arg)
	...
	def _process_lambda(query, func, globals, locals, order_by=False, original_names=False):
		...
		new_key = HashableDict(query._key, filters=query._key['filters'] + tup)
		new_filters = query._filters + (('apply_lambda', func_id, new_filter_num, order_by, func_ast, argnames,
										 original_names, extractors, None, vartypes),)

		new_translator, new_vars = query._get_translator(new_key, new_vars)
		if new_translator is None:
			prev_optimized = prev_translator.optimize
			new_translator = prev_translator.apply_lambda(func_id, new_filter_num, order_by, func_ast, argnames,
														  original_names, extractors, new_vars, vartypes)
			if not prev_optimized:
				...
				if name_path:
					...
					new_translator = new_translator.apply_lambda(func_id, new_filter_num, order_by, func_ast, argnames,
																 original_names, extractors, new_vars, vartypes)
			query._database._translator_cache[new_key] = new_translator
